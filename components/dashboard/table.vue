...
<template>
    <div class="px-4 py-3 @container">
        <div
            class="overflow-hidden rounded-xl border border-[#d0e7d9] dark:border-[#475569] bg-[#f8fcfa] dark:bg-[#1e293b]">
            <table class="w-full table-auto">
                <thead>
                    <tr class="bg-[#f8fcfa] dark:bg-[#1e293b]">
                        <th
                            class="px-4 py-3 text-left text-[#0e1b13] dark:text-[#cbd5e1] text-sm font-medium leading-normal uppercase">
                            Rank</th>
                        <th
                            class="pl-2 pr-6 py-3 text-left text-[#0e1b13] dark:text-[#cbd5e1] text-sm font-medium leading-normal uppercase">
                            Coordinator</th>
                        <th
                            class="px-4 py-3 text-left text-[#0e1b13] dark:text-[#cbd5e1] text-sm font-medium leading-normal uppercase">
                            Parish</th>
                ...
                <tbody>
                    <tr v-for="parish in paginatedItems" :key="parish.parish"
                        ...
                        <td
                            class="h-[72px] px-2 py-2 text-[#4e976d] dark:text-white text-sm font-normal leading-normal">
                            {{ parish.rank }}</td>
                        <td
                            class="h-[72px] pl-2 py-2 text-sm text-[#4e976d] dark:text-[#4ade80] text-sm font-normal leading-normal">
                            {{ parish.coordinator }}</td>
                        <td
                            class="h-[72px] px-4 py-2 text-[#0e1b13] dark:text-[#cbd5e1] text-sm font-normal leading-normal">
                            {{ parish.parish }}</td>
    ...
    <div class="relative mt-4 flex items-center justify-center space-x-1">
        ...
        <template v-for="page in paginationRange" :key="page">
            <span v-if="page === '...'"
                class="flex items-center justify-center w-8 h-8 text-sm font-medium text-gray-600">...</span>
            <button v-else @click="goToPage(page)"
                :class="{ 'bg-blue-500 text-white': currentPage === page, 'text-gray-600 bg-white hover:bg-blue-100': currentPage !== page }"
